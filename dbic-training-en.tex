...
MySQL databases should be created with proper UTF8 encoding and appropriate 
...
\item PlaceVisited => places_visited
\item User => users
\end{itemize}
\end{frame}
...
\item CRUD Basics
\item CRUD Methods
\item Search conditions and attributes
\item Chaining
\end{itemize}
\end{frame}
...
VisitedPlaces where home\_location is true:
...
VisitedPlaces where home\_location is false:
...
result. Using prefetch on places_visited means that all columns from the Location
class will be available via the places_visited accessor accessor for each user
object returned.
...
We want a list of all users complete with a count of places_visited for each. The
...
We cannot add other interesting data from the places_visited in the same query
...
the User object the count will not include the extra places_visited you just
...
In our example schema every user can have zero, one or many places_visited and
...
And since we expect each user to have many places_visited we define the relationship as a \verb|has_many|:
...
All places_visited visited before 1st January 2000.
...
\item We cannot add other interesting data from the places_visited in the same query (such as date of most recent visit).
\end{itemize}
